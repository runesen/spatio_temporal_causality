...
dat_covariates <- read.csv("PanelData_20180531.csv", header = TRUE)
...
shared_polygons <- Reduce(intersect, list(unique(dat_meta$UniqueID), 
                                          ...
                                          unique(dat_covariates$PolygonID))
                          )
...
dat_covariates <- subset(dat_covariates, PolygonID %in% shared_polygons)
...
dat_FL <- dat_FL[dat_FL$UniqueID,]
dat_conflict <- dat_conflict[order(dat_conflict$PolygonID, dat_conflict$Year),]
dat_covariates <- dat_covariates[order(dat_covariates$PolygonID, dat_covariates$Year),]
...
# distance to road
dat$RoadDist <- rep(subset(dat_covariates, Year == 2001)$RoadDist, each = nt)
...
write.table(dat0, "data_xy_colombia_20200327.txt", quote = FALSE)